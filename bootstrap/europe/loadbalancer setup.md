...
This guide installs the official AWS Load Balancer Controller Helm chart through the AWS-managed Argo CD capability for `production-europe-eks`.
...
`EKS` > `production-europe-eks` > `Capabilities` > `Argo CD` > `Open Argo CD UI`
...
```yaml
clusterName: production-europe-eks
region: eu-west-1
vpcId: vpc-02d1fc050beeae326
...
```powershell
aws eks update-kubeconfig --region eu-west-1 --name production-europe-eks
